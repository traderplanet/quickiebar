...
class QuickieBar
{
    ...
    public function include_before_plugin_loaded()
    {
        global $wpdb;
    ...
    public function include_after_plugin_loaded()
    {
        global $QB_VERSION;
    ...
    public static function initializeQBOptions()
    {
    ...
    public static function initializeQBDB()
    {
        global $wpdb;
    ...
    public static function destroyQBOptions()
    {
    ...
    public static function destroyQBDB()
    {
        global $wpdb;
    ...
    public static function deactivateAndDestroyQBData()
    {
        global $QB_VERSION;
    ...
    public static function load_quickiebar_script()
    {
        global $QB_VERSION;
    ...
    public static function should_load_quickiebar_script()
    {
        ...
        if ($visibility == 'everywhere') {
            return true;
        } elseif ($visibility == 'pagesonly' && (is_page() || is_home())) {//note that is_home() is required because a static blog page won't return true for is_page, but it will for is_home
            return true;
        } elseif ($visibility == 'postsonly' && is_singular('post')) {
            return true;
        } elseif ($visibility == 'homepageonly' && is_front_page()) {
            return true;
            ...
            if (is_page() || is_home()) {//note that is_home() is required because a static blog page won't return true for is_page, but it will for is_home
                ...
                elseif ($page_visibility == 'hide' && in_array($page_id, $page_exceptions)) {
                    return true;
                } else {
                    return false;
                }
                ...
                if ($post_visibility == 'hide' || count($post_exceptions) > 0) {
                    ...
                    //if page visibility is set to HIDE and page IS on the exceptions list
                    elseif ($post_visibility == 'hide' && in_array($post_id, $post_exceptions)) {
                        return true;
                    } else {
                        return false;
                    }
                } elseif ($category_visibility == 'hide' || count($category_exceptions) > 0) {
                    //if page visibility is set to SHOW and page IS NOT on the exceptions list
                    if ($category_visibility == 'show' && !$post_has_category_on_exception_list) {
                        return true;
                    }
                    //if page visibility is set to HIDE and page IS on the exceptions list
                    elseif ($category_visibility == 'hide' && $post_has_category_on_exception_list) {
                        return true;
                    } else {
                        return false;
                    }
            ...
            } elseif (is_singular() && !is_singular('post') && !is_page() && !is_attachment()) {
                //custom post type
